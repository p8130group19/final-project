...
dissimilarity is apparent. the percent of individuals with a bachelor's degree, shown by the polygon in red in high density areas differs by geographic region in the data. Color suggests
```{r contour}
contour_plot <-  ggplot(data=can_filt3[,c("poverty_percent","target_death_rate","pct_bach_deg25_over","pct_public_coverage_alone")] %>% head(), mapping= aes(x=fmsb::percentile(poverty_percent),
                                                              y=fmsb::percentile(target_death_rate),
                                                              z=fmsb::percentile(pct_bach_deg25_over),
#                                                              facet=state_region, 
                                                              color = fmsb::percentile(pct_public_coverage_alone))) +
  #geom_point()
  #stat_density_tern(aes(fill=..level.., alpha=..level..),geom='polygon') +
  stat_contour(geom="polygon", aes(z = fmsb::percentile(pct_bach_deg25_over), fill = fmsb::percentile(pct_bach_deg25_over),
                                  color= fmsb::percentile(pct_public_coverage_alone))) +
  scale_fill_gradient2(high = "red") +         
   geom_point() + facet_wrap(~state_region,ncol=2) + scale_color_viridis_c() + geom_density_tern() + theme_showarrows() +
  guides(fill = "none", alpha = "none") +
  labs (x = "Poverty", y = "Death Rate", z = "% Bachelor's Degree", color = "% Public Insurance Coverage Alone") +  theme(legend.position = "bottom") 
contour_plot
```
