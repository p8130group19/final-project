...
```
```{r lasso_diags}
plotmo::plotres(final_lasso_no_inc) #still having a couple issues with these. Feel free to try a different package if these don't work well.
plotmo::plotres(final_ridge_no_inc)
plot(full_lm_filtered)
plot(both_model)
```
```{r outliers}
outlier_counties <- can_reg_state$county[c(1221,1336,1490)]
