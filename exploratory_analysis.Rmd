...
X = as.matrix(can_filt %>% dplyr::select(-target_death_rate, -bladder, -ovary, -brca_cis, -cervix, -colorectal, -lungbronc, -melanoma, -ovary, -prostate) %>% sapply(as.numeric) %>% na.omit())
#Y = na.omit(can_filt)$target_death_rate
Y = as.matrix(can_filt %>% dplyr::select( -bladder, -ovary, -brca_cis, -cervix, -colorectal, -lungbronc, -melanoma, -ovary, -prostate) %>% sapply(as.numeric) %>% na.omit() %>% .[,"target_death_rate"])
...
```
As we take it out, we find that it gains a much lower importance in ridge regression. We also notice that our MSE becomes much higher and we could be overfitting our model.
```{r lasso_regression}
