...
upper_fit = 
  lm(target_death_rate ~ incidence_rate + poverty_percent + study_per_cap + median_age + avg_household_size + pct_hs25_over + pct_bach_deg25_over + pct_unemployed16_over +pct_public_coverage_alone + pct_white + pct_married_households + birth_rate, data = can_reg_omit)
...
`target_death_rate ~ pct_bach_deg25_over + incidence_rate + 
    poverty_percent + pct_hs25_over + pct_unemployed16_over + 
    pct_married_households + birth_rate + median_age + pct_public_coverage_alone`
which include 9 variables. 

```{r skim}
can_reg_omit %>% 
  select(pct_bach_deg25_over,incidence_rate, pct_married_households, 
...
```{r diagonostic plot}
forward1 = lm(formula = target_death_rate ~ pct_bach_deg25_over +incidence_rate+ pct_married_households + 
    pct_hs25_over + pct_unemployed16_over + birth_rate + median_age + 
...
cv_forward1 = 
  ...
  mutate(model = map(train, ~lm(formula = target_death_rate ~ pct_bach_deg25_over + incidence_rate + pct_married_households + 
    pct_hs25_over + pct_unemployed16_over + birth_rate + median_age + 
    pct_public_coverage_alone + avg_household_size, data = .x))) %>% 
  mutate(mse = map2_dbl(model, test, ~mse(model = .x, data = .y)))


mean(cv_forward1$mse) #422.2472
mse(model = forward1, data = can_reg_omit) #396.8848
```
