...
```
Utilizing lasso, we find that we have almost  precisely the same model. The dev ratio is nearly identical, at 0.3649.

With a dev ratio of 0.3649, this might be a reasonable model... but there may be far better ones from standard linear regression. We'll compare using R^2.  We'll perform the same selection of inital variables as before, or we'll have a fair number of counties ending up with missing data.
...
```
We'll find that the lasso selected model only has a modest change to the adjusted R^2 from the full filtered model and may perform better on test data than the full model (may be less overfit). With absolutely no fitering, the R^2=1 and the adjusted R^2 cannot be calculated.
```{r both_selection}
...
```
Doing the both selection, we'll find there's a single missing value for state_region. It should be obvious how to correct this. It's the district of columbia. Considering the state above and below the district are both considered to be in the "South" level (2), we'll assign the same region to it.
