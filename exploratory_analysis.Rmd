...
```
```{r write_arff}
can_reg_arff <- can_reg_joined %>% mutate(binned_inc = as.numeric(as.factor(binned_inc))) %>% dplyr::select(-county,-geography, -state_abb) %>% mutate(state = as.numeric(as.factor(state))) %>% dplyr::select(everything(),target_death_rate)
foreign::write.arff(x =  can_reg_arff, file = "can_reg_joinedv2.arff")
```
...
plotmo::plot_glmnet(final_ridge_range,xvar = c())
...
#car::vif(lm(data=can_filt,target_death_rate ~ .))
...
```{r ridge_outliers}
#not sure if we can even note this.
faraway::vif(lm(data=can_filt,target_death_rate ~ .)) %>% as.data.frame() %>% add_rownames() %>% as.tibble() %>% View()
...
# XMD$outlier <- "No"
# XMD$outlier[XMD > 20] <- "Yes"    # Threshold set to 20
 mdist_outliers<- chemometrics::Moutlier(X)
#which(mdist_outliers$rd>30)
...
# res_lasso_ls<- cbind(coef(mult.fit),coef(lasso3))
# colnames(res_lasso_ls) <- c("LS", "lasso")
...
```
With the inflated variables removed and a new dev ratio of 
```{r caret_cv}
# data_train<-trainControl(method="cv", number=10)
# model_caret<-train(X, Y,
#   #                 data=can_filt2,
...
#summary table for every variable.
# can_filt %>% 
#   group_by_all(state_region, mean)  
  

```
```{r plotting_diags}
plotmo::plotres(final_ridge)

object=final_ridge
 plotres<-  function (object = stop("no 'object' argument"), which = 1:4, 
    info = FALSE, versus = 1, standardize = FALSE, delever = FALSE, 
    level = 0, id.n = 3, labels.id = NULL, smooth.col = 2, grid.col = 0, 
    jitter = 0, do.par = NULL, caption = NULL, trace = 0, npoints = 3000, 
    center = TRUE, type = NULL, nresponse = NA, object.name = quote.deparse(substitute(object)), 
    ...) 
{
    plotmo:::init.global.data()
    on.exit({
        init.global.data()
        gc()
    })
    ...
    if (is.specified(grid.col) && is.logical(grid.col) && grid.col) 
        grid.col <- "lightgray"
    check.integer.scalar(nresponse, min = 1, na.ok = TRUE, logical.ok = FALSE, 
        char.ok = TRUE)
    temp <- get.plotres.data(object, object.name, which, standardize, 
        delever, level, versus, id.n, labels.id, trace, npoints, 
        type, nresponse, ...)
    nresponse <- temp$nresponse
    resp.name <- temp$resp.name
    type <- temp$type
    rinfo <- temp$rinfo
    vinfo <- temp$vinfo
    fitted <- temp$fitted
    which <- temp$vinfo$which
    id.n <- temp$id.n
    ...
    do.par <- check.do.par(do.par, nfigs)
    caption <- get.caption(nfigs, do.par, caption, resp.name, 
        type, getCall(object), object.name, my.call)
    if (do.par) {
        oldpar <- par(no.readonly = TRUE)
        do.par(nfigs = nfigs, caption = caption, main1 = NA, 
            xlab1 = dota("xlab", DEF = NULL, ...), ylab1 = dota("ylab", 
                DEF = NULL, ...), trace = trace, nlines.in.main = nlines.in.plotres.main(object = object, 
                which = which, versus = versus, standardize = standardize, 
                delever = delever, level = level, ...), def.font.main = 1, 
            ...)
        if (do.par == 1) 
    ...
    force.auto.resids.xlim <- length(which) > 1 && (W1 %in% which) && 
        inherits(object, "earth") && !is.dot("w1.xlim", ...)
    force.auto.resids.ylim <- length(which) > 1 && (W1 %in% which) && 
        inherits(object, "earth") && !is.dot("w1.ylim", ...)
    w1.retval <- list(plotted = FALSE, retval = NULL)
    w3.retval <- NULL
    attempted.w1.plot <- FALSE
    if (any(which == W1)) {
        w1.retval <- plot_w1(object = object, which = which, 
            info = info, standardize = standardize, delever = delever, 
            level = level, versus = versus, id.n = id.n, labels.id = rinfo$labs, 
            smooth.col = smooth.col, grid.col = grid.col, do.par = do.par, 
            caption = if (all(which == W1)) 
                caption
            else "", trace = trace, npoints = npoints, center = center, 
            type = type, nresponse = nresponse, object.name = object.name, 
            ...)
        attempted.w1.plot <- TRUE
        which <- which[which != W1]
        if (length(which) == 0 && !w1.retval$plotted && trace >= 
            0) 
            warning0("plotres: nothing to plot")
    }
    if (length(which) == 0) 
    ...
    iresids <- get.isubset(rinfo$resids, npoints, id.n, use.all = (vinfo$nversus == 
        V4LEVER), rinfo$scale)
    xlim <- dota("xlim", DEF = NULL, ...)
    for (icolumn in vinfo$icolumns) {
        for (iwhich in seq_along(which)) {
            if (which[iwhich] == W2CUM) 
                plotmo_cum(rinfo = rinfo, info = info, nfigs = nfigs, 
                  add = FALSE, cum.col1 = NA, grid.col = grid.col, 
                  jitter = 0, ...)
                ...
                plotmo_qq(rinfo = rinfo, info = info, nfigs = nfigs, 
                  ...
                  ...)
            else w3.retval <- plotresids(object = object, which = which[iwhich], 
                info = info, standardize = standardize, level = level, 
                versus1 = vinfo$versus.mat[, icolumn], id.n = id.n, 
                smooth.col = smooth.col, grid.col = grid.col, 
                jitter = jitter, npoints = npoints, center = center, 
                type = type, fitted = fitted, rinfo = rinfo, 
                rsq = rsq, iresids = iresids, nversus = vinfo$nversus, 
