...
#   mutate(pct_unemployed16_over = (pct_unemployed16_over)^.5,
#          poverty_percent = log(poverty_percent + 1e-2),
# #         study_per_cap = (study_per_cap)^(-1/15),
#          birth_rate = (birth_rate)^.5,
#          incidence_rate = log(incidence_rate + 1e-2),
#          lungbronc = log(lungbronc + 1e-2),
#          avg_household_size = (avg_household_size)^2,
#          pct_white = (pct_white)^3,
#          pct_bach_deg25_over = (pct_bach_deg25_over)^.5)
...
skew_table %>% 
  ...
  arrange(-abs_skew)
#which(sapply(can_filt3, is.numeric))
#boxcox_plots<-foreach(i=which(sapply(can_filt3, is.numeric))) %do%
#{lm(data= can_filt3, as.formula(paste0("target_death_rate ~ ",paste0(names(can_filt3)[i])))) %>% MASS::boxcox(ylab=names(can_filt3)[i])}
#hist(log(can_filt3$study_per_cap+0.01))
#hist(can_filt3$pct_white^(1/10))
...
plotmo::plotres(final_lasso_range, info = T,which = 4) #non matter how many outliers, I remove, they always come back.
```
